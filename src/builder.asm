	; build
	; SNA_FILENAME defined in Makefile
	; TRD_FILENAME defined in Makefile
...
EndBasic:
	...
	savebin "build/page7.c", page7s, page7e-page7s
	endif

	ifdef _page6
	page 6
	savetrd TRD_FILENAME, "6.C", page6s, page6e-page6s
	savebin "build/page6.c", page6s, page6e-page6s
	endif
	
	ifdef _page4
	page 4
	savetrd TRD_FILENAME, "4.C", page4s, page4e-page4s
	savebin "build/page4.c", page4s, page4e-page4s
	endif
	
	ifdef _page3
	page 3
	savetrd TRD_FILENAME, "3.C", page3s, page3e-page3s
	savebin "build/page3.c", page3s, page3e-page3s
	endif
	
	ifdef _page1
	page 1
	savetrd TRD_FILENAME, "1.C", page1s, page1e-page1s
	savebin "build/page1.c", page1s, page1e-page1s
	endif
